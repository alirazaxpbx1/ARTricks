...
# ==============================================================================
# Script Name   : ARTricks VPN Panel (Dynamic Domain Supported)
# Custom Path   : /
# ==============================================================================
...
PANEL_NAME="ARTricks VPN Panel"
BANNER_FILE="/etc/issue.net"
CUSTOM_PATH="/ARTricks"
DOMAIN_FILE="/etc/ARTricks/domain.conf"
...
[Unit]
Description=ARTricks Auto-Kill & Bandwidth Tracking Service
...
PANEL_NAME = "ARTricks VPN Panel"
CONFIG_FILE = "/etc/ARTricks/tgbot/config.json"
ADMINS_FILE = "/etc/ARTricks/tgbot/admins.json"
USERS_DIR = "/etc/ARTricks/users"
DOMAIN_FILE = "/etc/ARTricks/domain.conf"
BANNER_FILE = "/etc/issue.net"
...
def set_domain(new_domain):
    os.makedirs("/etc/ARTricks", exist_ok=True)
...
Description=ARTricks WebSocket Proxy Service
After=network.target
...
DEFAULT_BANNER = (
    ...
    '<font color="yellow"><b>WELCOME TO ARTricks VIP VPN</b></font><br>\n'
...
[Unit]
Description=ARTricks Telegram Bot
After=network.target
...
check_gb_usage() {
    ...
    mkdir -p /etc/ARTricks/users

    for conf in /etc/ARTricks/users/*.conf; do
        [[ -e "$conf" ]] || continue
...
case $u_choice in
            1)
                ...
                mkdir -p /etc/ARTricks/users
                echo "IP_LIMIT=$ip_limit" > "/etc/ARTricks/users/${username}.conf"
...
press_any_key
            ...
            2)
                ...
                rm -f "/etc/ARTricks/users/${username}.conf"
            ...
            6)
                ...
                if [[ -f "/etc/ARTricks/users/${username}.conf" ]]; then
                    read -rp "Nayi IP Limit enter karein (e.g. 2 ya 3): " new_ip_l
                    sed -i "s/IP_LIMIT=.*/IP_LIMIT=${new_ip_l}/g" "/etc/ARTricks/users/${username}.conf"
                    passwd -u "$username" 2>/dev/null
            ...
            7)
                ...
                if [[ -f "/etc/ARTricks/users/${username}.conf" ]]; then
                    ...
                    sed -i "s/GB_LIMIT=.*/GB_LIMIT=${new_gb}/g" "/etc/ARTricks/users/${username}.conf"
                    passwd -u "$username" 2>/dev/null
...
setup_telegram_bot() {
    ...
    read -rp "${YELLOW}${BOLD}  ➜ Select Option [1-5]: ${NC}" tb_opt
        1)
            ...
            mkdir -p /etc/ARTricks/tgbot
            cat << CFG_EOF > /etc/ARTricks/tgbot/config.json
{"token": "${bot_token}", "super_admin": ${super_id}}
CFG_EOF
            echo "[${super_id}]" > /etc/ARTricks/tgbot/admins.json
